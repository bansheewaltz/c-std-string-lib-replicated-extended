...
void parse_value(specifications *specs, char *buff, va_list *va) {
  ...
  if (x_ischarset(specs->specifier, 3, 'E', 'G', 'X')) {
    to_upper(buff);
  }
}
...
bool is_integer_specifier(char c) {
  return x_ischarset(c, 6, 'd', 'i', 'o', 'u', 'x', 'X');
}
...
void format_flags(char *buff, specifications *specs) {
  ...
  if (specs->plus && specs->specifier != 'u') {
    tmp[0] = buff[0] == '-' ? buff[0] : '+';
    x_strcpy(tmp + 1, buff[0] == '-' ? buff + 1 : buff);
    x_strcpy(buff, tmp);
  ...
  if (specs->width > (int)x_strlen(buff)) {
    int idx = specs->width - x_strlen(buff);
    if (!specs->minus) {
      x_memset(tmp, specs->zero ? '0' : ' ', idx);
      x_strcpy(tmp + idx, buff);
    } else {
      x_strcpy(tmp, buff);
      x_memset(tmp + x_strlen(tmp), ' ', idx);
    }
    x_strcpy(buff, tmp);
  }
}
...
void parse_octal(specifications *specs, char *buff, va_list *va) {
  buff[0] = '0';
  x_itoa(va_arg(*va, int64_t), buff + specs->hash, 8);
  format_precision(buff, specs);
  format_flags(buff, specs);
}
...
void prepend_ox(char *buff, specifications *specs) {
  if (!is_all_zeroes(buff) || specs->specifier == 'p') {
    x_memmove(buff + 2, buff, x_strlen(buff));
    buff[0] = '0';
    buff[1] = 'x';
  }
}
...
void format_string(specifications *specs, char *buff, char *str) {
  ...
  int shift = specs->width - x_strlen(tmp);
  int len = x_strlen(tmp);

  if (specs->minus && shift > 0) {
    x_strcpy(buff, tmp);
    x_memset(buff + len, ' ', shift);
  } else if (shift > 0) {
    x_memset(buff, ' ', shift);
    x_strcpy(buff + shift, tmp);
  } else {
    x_strcpy(buff, tmp);
  }
}
...
void parse_pointer(specifications *specs, char *buff, va_list *va) {
  x_uitoa(va_arg(*va, uint64_t), buff, 16);
  format_precision(buff, specs);
  prepend_ox(buff, specs);
  format_flags(buff, specs);
}
...
void format_gG_precision(char *buff, int precision) {
  int sig_digs = 0;
  x_size_t len = x_strlen(buff);
  int not_zero_found = 0;

  for (x_size_t i = 0; i < x_strlen(buff); i++) {
