...
void x_dtostr(long double val, char *ret, specifications *specs) {
  char buff[BUFF_SIZE] = "";
  int idx = BUFF_SIZE - 2;
  bool neg = val < 0 ? 1 : 0;
  val = neg ? val * -1 : val;
  ...
  if (!right) {
    ...
    for (int i = x_strlen(fractions); right || i > 0; right /= 10, idx--, i--)
      buff[idx] = (int)(right % 10 + 0.05) + '0';
  }
  if ((specs->is_precision_set && specs->precision != 0) || (int)r ||
      (!specs->is_precision_set && val == 0) || x_strlen(fractions))
    buff[idx--] = '.';
  if (!left) {
    buff[idx] = '0';
    idx--;
