...
int x_sscanf(const char *restrict s, const char *restrict format, ...) {
  va_list ptrs;
  ...
  while ((*f_ptr != '\0') && (*s_ptr != '\0')) {
    if (*f_ptr != '%') {
      ...
      options.width = (x_size_t)x_strtol(f_ptr, &f_ptr, 10);
      if (x_strchr("hlL", *f_ptr) != 0) {
      ...
      switch (options.specifier) {
        ...
        case 'd':
          if (options.precission == 0) {
            if (options.length == 'h') {
              result += scan_hint(&s_ptr, options, x_NULL);
            } else if (options.length == 'l') {
              result += scan_lint(&s_ptr, options, x_NULL);
            } else if (options.length == '\0') {
              result += scan_int(&s_ptr, options, x_NULL);
            }
          } else {
            if (options.length == 'h') {
        ...
        case 'X':
          if (options.precission == 0) {
            if (options.length == 'h') {
              result += scan_uhint(&s_ptr, options, x_NULL);
            } else if (options.length == 'l') {
              result += scan_ulint(&s_ptr, options, x_NULL);
            } else if (options.length == '\0') {
              result += scan_uint(&s_ptr, options, x_NULL);
            }
          } else {
            if (options.length == 'l') {
        ...
        case 's':
          if (options.precission == 0) {
            result += scan_char(&s_ptr, options, x_NULL);
          } else {
        ...
        case 'G':
          if (options.precission == 0) {
            if (options.length == '\0') {
              result += scan_float(&s_ptr, options, x_NULL);
            } else if (options.length == 'l') {
              result += scan_double(&s_ptr, options, x_NULL);
            } else if (options.length == 'L') {
              result += scan_ldouble(&s_ptr, options, x_NULL);
            }
          } else {
            if (options.length == '\0') {
        ...
        case 'p':
          if (options.precission == 0) {
            result += scan_ptr(&s_ptr, options, x_NULL);
          } else {
...
int scan_char(char **str, struct opt options, char *dst) {
  if (options.specifier == 'c') {
    ...
    *str += options.width;
    ...
    if (dst != x_NULL) {
      x_strncpy(dst, *str, options.width);
      dst[options.width] = '\0';
    }
    *str += options.width;
  }
...
int scan_hint(char **str, struct opt options, short int *dst) {
  ...
  if (options.width != 0) {
    ...
    if (NULL != tmp_ptr) {
      x_strncpy(tmp_ptr, *str, options.width);
      result = (short)x_strtol(tmp_ptr, x_NULL, base);
      free(tmp_ptr);
      tmp_ptr = x_NULL;
      *str += options.width;
    }
  } else if (options.width == 0) {
    result = (short)x_strtol(*str, str, base);
  }
  if (dst != x_NULL) {
    *dst = result;
  }
...
int scan_uhint(char **str, struct opt options, unsigned short int *dst) {
  ...
  if (options.width != 0) {
    ...
    if (NULL != tmp_ptr) {
      x_strncpy(tmp_ptr, *str, options.width);
      result = (unsigned short)x_strtol(tmp_ptr, x_NULL, base);
      free(tmp_ptr);
      tmp_ptr = x_NULL;
      *str += options.width;
    }
  } else if (options.width == 0) {
    result = (unsigned short)x_strtol(*str, str, base);
  }
  if (dst != x_NULL) {
    *dst = result;
  }
...
int scan_int(char **str, struct opt options, int *dst) {
  ...
  if (options.width != 0) {
    ...
    if (NULL != tmp_ptr) {
      x_strncpy(tmp_ptr, *str, options.width);
      result = (int)x_strtol(tmp_ptr, x_NULL, base);
      free(tmp_ptr);
      tmp_ptr = x_NULL;
      *str += options.width;
    }
  } else if (options.width == 0) {
    result = (int)x_strtol(*str, str, base);
  }
  if (dst != x_NULL) {
    *dst = result;
  }
...
int scan_uint(char **str, struct opt options, unsigned int *dst) {
  ...
  if (options.width != 0) {
    ...
    if (NULL != tmp_ptr) {
      x_strncpy(tmp_ptr, *str, options.width);
      result = (unsigned int)x_strtol(tmp_ptr, x_NULL, base);
      free(tmp_ptr);
      tmp_ptr = x_NULL;
      *str += options.width;
    }
  } else if (options.width == 0) {
    result = (unsigned int)x_strtol(*str, str, base);
  }
  if (dst != x_NULL) {
    *dst = result;
  }
...
int scan_lint(char **str, struct opt options, long int *dst) {
  ...
  if (options.width != 0) {
    ...
    if (NULL != tmp_ptr) {
      x_strncpy(tmp_ptr, *str, options.width);
      result = x_strtol(tmp_ptr, x_NULL, base);
      free(tmp_ptr);
      tmp_ptr = x_NULL;
      *str += options.width;
    }
  } else if (options.width == 0) {
    result = x_strtol(*str, str, base);
  }
  if (dst != x_NULL) {
    *dst = result;
  }
...
int scan_ulint(char **str, struct opt options, unsigned long int *dst) {
  ...
  if (options.width != 0) {
    ...
    if (NULL != tmp_ptr) {
      x_strncpy(tmp_ptr, *str, options.width);
      result = (unsigned long)x_strtol(tmp_ptr, x_NULL, base);
      free(tmp_ptr);
      tmp_ptr = x_NULL;
      *str += options.width;
    }
  } else if (options.width == 0) {
    result = (unsigned long)x_strtol(*str, str, base);
  }
  if (dst != x_NULL) {
    *dst = result;
  }
...
int scan_float(char **str, struct opt options, float *dst) {
  ...
  if (options.width != 0) {
    ...
    if (NULL != tmp_ptr) {
      x_strncpy(tmp_ptr, *str, options.width);
      result = (float)x_strtod(tmp_ptr, x_NULL);
      free(tmp_ptr);
      tmp_ptr = x_NULL;
      *str += options.width;
    }
  } else if (options.width == 0) {
    result = (float)x_strtod(*str, str);
  }
  if (dst != x_NULL) {
    *dst = result;
  }
...
int scan_double(char **str, struct opt options, double *dst) {
  ...
  if (options.width != 0) {
    ...
    if (NULL != tmp_ptr) {
      x_strncpy(tmp_ptr, *str, options.width);
      result = (double)x_strtod(tmp_ptr, x_NULL);
      free(tmp_ptr);
      tmp_ptr = x_NULL;
      *str += options.width;
    }
  } else if (options.width == 0) {
    result = (double)x_strtod(*str, str);
  }
  if (dst != x_NULL) {
    *dst = result;
  }
...
int scan_ldouble(char **str, struct opt options, long double *dst) {
  ...
  if (options.width != 0) {
    ...
    if (NULL != tmp_ptr) {
      x_strncpy(tmp_ptr, *str, options.width);
      result = x_strtod(tmp_ptr, x_NULL);
      free(tmp_ptr);
      tmp_ptr = x_NULL;
      *str += options.width;
    }
  } else if (options.width == 0) {
    result = x_strtod(*str, str);
  }
  if (dst != x_NULL) {
    *dst = result;
  }
...
int scan_ptr(char **str, struct opt options, void **dst) {
  ...
  if (options.width != 0) {
    ...
    if (NULL != tmp_ptr) {
      x_strncpy(tmp_ptr, *str, options.width);
      result = (void *)x_strtol(tmp_ptr, x_NULL, base);
      free(tmp_ptr);
      tmp_ptr = x_NULL;
      *str += options.width;
    }
  } else if (options.width == 0) {
    result = (void *)x_strtol(*str, str, base);
  }
  if (dst != x_NULL) {
    *dst = result;
  }
